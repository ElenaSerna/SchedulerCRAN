...
//	    schedulerDRANMac::Nsubbands_user=NsubbandsUser;
	  Config::SetDefault ("ns3::schedulerCRANMac::FilesEnabled", BooleanValue (generateSchedFiles));
	  Config::SetDefault ("ns3::schedulerCRANMac::MaxSubbandsUE", UintegerValue (NsubbandsUser));
	  Config::SetDefault ("ns3::LteEnbMac::Centralized", BooleanValue(true));
	  Config::SetDefault ("ns3::LteEnbPhy::Centralizado", BooleanValue(true));

	    LteEnbMac::ULDLselection=ULDL; // DL - 0 ; UL - 1
//	    LteEnbMac::m_centralized=true; // DL - 0 ; UL - 1
  ...
  lteHelper->SetSchedulerType("ns3::schedulerCRANMac"); // Distribuido
...
//  uint16_t numberOfUeNodes = 8;
  ...
  enbNodes.Create (2); // Una celda sera la "hipercelda" a la que se conectaran todos los UEs y se realizara la asignacion de recursos centralizada
  ...
  for (uint16_t i = 0; i < enbNodes.GetN(); i++)
    {
 	  ...
 	  Ptr<MobilityModel> mm = enb->GetObject<MobilityModel>();
 	  std::cout << "eNB " << i << " pos: " << mm->GetPosition()<< std::endl;
...
// 	  //	  pos.z=1.5;
 	  std::cout << "UE " << i << " pos: " << mm->GetPosition()<< std::endl;
   }

  // Install LTE Devices to the nodes -- CLUSTER
...
//  NetDeviceContainer ueDevs = lteHelper->InstallUeDevice (ueNodes);
  ...
  // Attachamos todos los ues a la celda 1 (hipercelda)
  for (uint16_t i=0; i<ueDevs.GetN(); i++)
  {
...
//  lteHelper->Attach(ueDevs.Get(2), enbDevs.Get(1));
  ...
  Ptr<LteEnbRrc> rrc1 = enbDevs.Get(0)->GetObject<LteEnbNetDevice>()->GetRrc();
  Ptr<LteEnbRrc> rrc2 = enbDevs.Get(1)->GetObject<LteEnbNetDevice>()->GetRrc();
  Ptr<LteEnbMac> mac1 = enbDevs.Get(0)->GetObject<LteEnbNetDevice>()->GetMac();
  Ptr<LteEnbMac> mac2 = enbDevs.Get(1)->GetObject<LteEnbNetDevice>()->GetMac();

//  mac2->m_hipercell=true;
  Ptr<LteEnbMac> hipercellMac = enbDevs.Get(0)->GetObject<LteEnbNetDevice>()->GetMac();
  hipercellMac->DoSetListEnbRrc(rrc1);
  hipercellMac->DoSetListEnbRrc(rrc2);
  hipercellMac->DoSetListEnbMac(mac1);
  hipercellMac->DoSetListEnbMac(mac2);
  hipercellMac->m_hipercell=true;

  centralizado=true; // Ponemos variable global centralizado a true para que la puedan utilizan todas las clases
  // Añadimos los ues a la capa Mac de las celdas para que esten disponibles en el scheduler
  hipercellMac->SetUeDevices(ueDevs);

//  Ptr<Objetos> listObjects = CreateObject<Objetos> ();
//  listObjects->SetLteEnbRrcObject(rrc1);
//  listObjects->SetLteEnbRrcObject(rrc2);
//
//  std::vector <Ptr<LteEnbRrc>> lista;
//  lista.push_back(rrc1);
//  lista.push_back(rrc2);
//
//  ObjectPtrContainerValue lista2;
//  lista2=(ObjectPtrContainerValue)lista;

//  lista2.insert(rrc1);
//
//  Config::SetDefault ("ns3::schedulerDRANMac::Objects", ObjectVectorValue (lista));
