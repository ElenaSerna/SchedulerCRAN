...
void handover(NetDeviceContainer ues, NetDeviceContainer enbs)
{
	// Preparamos el evento de llamada a la funcion de handover
	// Seleccionamos la enb source
	Ptr <NetDevice> enbDeviceSource = enbs.Get(0);
	Ptr <NetDevice> enbDeviceTarget = enbs.Get(4);
	Ptr <NetDevice> ueDevice = ues.Get(0);


	Ptr <LteUeNetDevice> ue = ueDevice -> GetObject<LteUeNetDevice>();
	Ptr <LteEnbNetDevice> enbTarget = enbDeviceTarget-> GetObject<LteEnbNetDevice>();

	//
	Ptr<LteEnbRrc> enbRrcSource = enbDeviceSource->GetObject<LteEnbNetDevice> ()->GetRrc ();
	Ptr<LteEnbRrc> enbRrcTarget = enbDeviceTarget->GetObject<LteEnbNetDevice> ()->GetRrc ();
    Ptr<LteUeRrc> ueRrc = ueDevice->GetObject<LteUeNetDevice> ()->GetRrc ();
    uint16_t imsi=ue->GetImsi();
    uint16_t cellId=enbTarget->GetCellId();
    std::cout << imsi << cellId << std::endl;
    enbRrcSource->SendHandoverRequest(imsi,cellId);
//	Simulator::Schedule (Simulator::Now(), &LteEnbRrc::SendHandoverRequest,imsi,cellId);
...
//  Config::SetDefault ("ns3::PfFfMacSchedulerComparative::FilesEnabled", BooleanValue (generateSchedFiles));
  Config::SetDefault ("ns3::PfFfMacScheduler::FilesEnabled", BooleanValue (generateSchedFiles));
...
//  Config::SetDefault ("ns3::PfFfMacSchedulerComparative::MaxSubbandsUE", UintegerValue (NsubbandsUser));
  Config::SetDefault ("ns3::PfFfMacScheduler::MaxSubbandsUE", UintegerValue (NsubbandsUser));
...
//
    ...
    // Forzamos el attach del usuario 1 a la celda 1 para comprobar a posteriori el handover
//    lteHelper->Attach(ueDevs.Get(0), enbDevs.Get(0));

    // Programamos el evento que generara el handover
//    double time2=0.028;
//    Time t = Seconds(time2);
//    Simulator::Schedule(t, &handover, ueDevs, enbDevs);
