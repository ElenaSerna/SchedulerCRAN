                      ...
                      std::string path, PhyTransmissionStatParameters params)
{
  ...
  if (params.m_imsi==0) // En el caso del scheduler distribuido el imsi se tiene que buscar en el UeMap de la capa RRC de la eNB correspondiente
  {
                      ...
                      std::string path, PhyTransmissionStatParameters params)
{
  ...
  uint64_t imsi = 0;
  std::ostringstream pathAndRnti;
//  if (params.m_imsi==0) // Para scheduler distribuido
//  {
	  pathAndRnti << path << "/" << params.m_rnti;
	  if (phyTxStats->ExistsImsiPath (pathAndRnti.str ()) == true)
	    {
