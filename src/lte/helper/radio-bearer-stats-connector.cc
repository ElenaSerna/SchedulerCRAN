  ...
  if (!m_connected)
    {
      ...
      /////// AÑADIDAS PARA CENTRALIZADO ////////
            Config::Connect ("/NodeList/*/DeviceList/*/LteEnbRrc/ResetUeTraces",
		       MakeBoundCallback (&RadioBearerStatsConnector::ResetUeTraces, this));

      Config::Connect ("/NodeList/*/DeviceList/*/LteUeRrc/Disconnect",
		       MakeBoundCallback (&RadioBearerStatsConnector::DisconnectTraces, this));
      //////////////////////////////////////////
...
void
RadioBearerStatsConnector::ResetUeTraces (RadioBearerStatsConnector* c, std::string context, uint64_t imsi)
{
  c->Reset (context, imsi);
}

void
RadioBearerStatsConnector::DisconnectTraces (RadioBearerStatsConnector* c, std::string context, uint64_t imsi, uint16_t cellId, uint16_t rnti)
{
	c->DisconnectTracesHipercell(context, imsi, cellId, rnti);
}
...
////////////////// AÑADIDO PARA SCHEDULER CENTRALIZADO /////////////////////
void
RadioBearerStatsConnector::Reset(std::string context, uint64_t imsi)
{
	m_imsiSeenUe.erase(imsi);
	m_imsiSeenEnb.erase(imsi);
}


void
RadioBearerStatsConnector::DisconnectTracesHipercell(std::string context, uint64_t imsi, uint16_t cellId, uint16_t rnti)
{
...
//  std::string basePath = context.substr (0, context.rfind ("/"));
  std::ostringstream hipercellPath;
  hipercellPath <<  "/NodeList/0/DeviceList/0/LteEnbRrc/UeMap/" << (uint32_t) rnti;
