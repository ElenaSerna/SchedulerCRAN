...
public:
  ...
  //// AÑADIDO PARA SCHEDULER CENTRALIZADO /////
  virtual void AddLcCentralizado (LcInfo lcinfo, LteMacSapUser* msu);
  virtual void SetUe (uint16_t rnti);
  /////////////////////////////////////////////
...
////// AÑADIDO PARA SCHEDULER CENTRALIZADO //////
void
EnbMacMemberLteEnbCmacSapProvider::AddLcCentralizado (LcInfo lcinfo, LteMacSapUser* msu)
{
	m_mac->DoAddLcCentralizado (lcinfo, msu);
}

void
EnbMacMemberLteEnbCmacSapProvider::SetUe (uint16_t rnti)
{
  m_mac->DoSetUe (rnti);
}
////////////////////////////////////////////////
...
public:
  ...
  ///// Añadido para scheduler /////
  virtual void SetCellId (uint16_t cellId); // Añadida para introducir el CellId
  /////////////////////////////////
...
//// Añadido para scheduler ////
void
EnbMacMemberLteEnbPhySapUser::SetCellId (uint16_t cellId)
{
...
LteEnbMac::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::LteEnbMac")
	...
	.AddAttribute ("Centralized",
				   "Indicates whether there exists a centralized scenario",
				   BooleanValue (false),
				   MakeBooleanAccessor (&LteEnbMac::m_centralized),
				   MakeBooleanChecker ())

	.AddAttribute ("HiperCell",
			       "Indicates whether this eNB is the hipercell in the centralized scheduler",
				   BooleanValue (false),
				   MakeBooleanAccessor (&LteEnbMac::m_hipercell),
				   MakeBooleanChecker ())
...
int LteEnbMac::ULDLselection;
//bool LteEnbMac::m_centralized;

LteEnbMac::LteEnbMac ()
{
  NS_LOG_FUNCTION (this);
  m_updateStatusForCentralized=false;
...
///// Añadido para scheduler-> OBTENER CELL ID ///////
void
...
void
LteEnbMac::DoSetListEnbRrc(Ptr<LteEnbRrc> rrc)
{
	m_listLteEnbRrc.push_back(rrc);
}

void
LteEnbMac::DoSetListEnbMac(Ptr<LteEnbMac> mac)
{
	m_listLteEnbMac.push_back(mac);
}

void
LteEnbMac::SetUeDevices(NetDeviceContainer ueDevs)
{
	m_ueDevs=ueDevs;
}
...
void
LteEnbMac::UpdateRlcBufferStatusForCentralized(FfMacSchedSapProvider* hiperCellSapProvider)
{
	m_hiperCellSchedSapProvider = hiperCellSapProvider;
	m_updateStatusForCentralized=true;
}

std::map <uint16_t, std::map<uint8_t, LteMacSapUser*> >
LteEnbMac::GetRlcAttached()
{
	return m_rlcAttached;
}
/////////////////////////////////////////////////////
...
LteEnbCmacSapUser*
LteEnbMac::GetLteEnbCmacSapUser (void)
{
...
///////// EJECUTA SCHEDULER UL & DL ////////////
void
LteEnbMac::DoSubframeIndication (uint32_t frameNo, uint32_t subframeNo)
{


 //// Esta funcion es la encargada de lanzar el scheduler tanto en UL como en DL
  // Comprobamos si se trata de un escenario centralizado o distribuido
  // en ese caso comprobamos si la celda que va a ejecutar es la hipercelda
  ...
  case 0:  // --- DOWNLINK ---
  {
		  ...
		  if (!m_receivedRachPreambleCount.empty ())
			{
			  ...
			  for (std::map<uint8_t, uint32_t>::const_iterator it = m_receivedRachPreambleCount.begin ();
				   it != m_receivedRachPreambleCount.end (); // 1er elemento del mapa, rapID (Id del preambulo), segundo elemento, el numero de veces que se ha recibido
				   ++it)
				{
				  ...
				  if (it->second > 1)
				  ...
				  else
					{
					  ...
					  if (jt != m_allocatedNcRaPreambleMap.end ()) // For non-contention based Random Access procedure
						  // Cuando un UE accede por 1a vez a la red, envia un patron espeficio que se conoce como "Preambulo RACH"
						  // Este preambulo tiene un valor diferente por cada UE (existen hasta 64 valores diferentes) pero puede darse el caso de que
						  // dos UEs escojan el mismo preambulo y se produzca una colision. De modo que existen dos modos de elegir el preambulo
						  // a) Contention Based: el usuario escoge el preambulo
						  // b) Non-contention based: es la red la que informara al UE sobre que preambulo utilizar
						{
						  rnti = jt->second.rnti;
		  ...
		  //Incluimos el cellId y el RNTI
		  dlparams.m_cellId=m_cellId;
		  dlparams.m_hipercell=m_hipercell;
		  if (m_hipercell)
		  {
//			  std::pair<uint16_t, Ptr<UeManager>>::iterator it;
...
//			  }
		  ...
		  for (uint16_t i=0; i<m_listLteEnbRrc.size(); i++)
		  {
			  dlparams.m_mapEnbRrc.push_back(m_listLteEnbRrc.at(i));
			  dlparams.m_mapEnbMac.push_back(m_listLteEnbMac.at(i));
		  }

		  std::vector <MacCeListElement_s> uesRNTIs = this->m_ulCeReceived;
//				  for (int i=0; i<uesRNTIs.end(); i++)
//					  dlparams.m_rnti.push_back(uesRNTIs.at(i).m_rnti);
//				  }
//		  if (this->m_ulCeReceived>0)
//		  {
//		  std::cout << "rntis " << this->m_ulCeReceived.at(0).m_rnti << " to " << this->m_ulCeReceived.at(0).m_rnti << std::endl;
//		  }
		  //  dlparams.m_rnti.push_back(uesRNTIs.at(0).m_rnti);
		  for (uint16_t i=0; i<uesRNTIs.size(); i++)
		  {
			  dlparams.m_rnti.push_back(uesRNTIs.at(i).m_rnti);
		  }


		  // Diferenciamos entre un escenario centralizado y distribuido, en el caso del centralizado
		  // solo ejecutaremos el scheduler en la hipercelda
		  if (m_centralized && m_hipercell)
		  {
			 m_schedSapProvider->SchedDlTriggerReq (dlparams);

		  }
		  else if(!m_centralized)
			 m_schedSapProvider->SchedDlTriggerReq (dlparams);

		  dlparams.m_rnti.erase(dlparams.m_rnti.begin(), dlparams.m_rnti.end());
  ...
  default:
	  NS_FATAL_ERROR ("Debe especificarse el enlace UL o DL sobre el que se realizar la simulacion");
	  break;
...
////// AÑADIDO PARA SCHEDULER CENTRALIZADO //////
void
LteEnbMac::DoSetUe (uint16_t rnti)
{
  NS_LOG_FUNCTION (this << " rnti=" << rnti);
  std::map<uint8_t, LteMacSapUser*> empty;
  std::pair <std::map <uint16_t, std::map<uint8_t, LteMacSapUser*> >::iterator, bool>
...
void
LteEnbMac::DoAddLcCentralizado (LteEnbCmacSapProvider::LcInfo lcinfo, LteMacSapUser* msu)
{
  NS_LOG_FUNCTION (this);
...
LteEnbMac::DoReportBufferStatus (LteMacSapProvider::ReportBufferStatusParameters params)
{
  ...
  ////// PARA EL CASO CENTRALIZADO, ENVIAMOS LA ACTUALIZACIÒN DEL BUFFER RLC AL SCHEDULER DE LA HIPERCELDA //////
  if (m_centralized && m_updateStatusForCentralized)
      m_hiperCellSchedSapProvider->SchedDlRlcBufferReq(req);
  else
...
LteEnbMac::DoSchedDlConfigInd (FfMacSchedSapUser::SchedDlConfigIndParameters ind)
{
  ...
  for (unsigned int i = 0; i < ind.m_buildDataList.size (); i++) // Recorremos el numero de usuarios
    {
