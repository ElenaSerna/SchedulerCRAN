...
public:
  ...
  static int ULDLselection; // UL - 1; DL - 0
  uint16_t m_cellId;
  bool m_centralized; // Para seleccionar entre un escenario centralizado y distribuido
  bool m_hipercell;
  std::vector <Ptr<LteEnbRrc>> m_listLteEnbRrc;
  std::vector <Ptr<LteEnbMac>> m_listLteEnbMac;
  std::map<uint16_t, uint64_t> m_rntiImsiRelation;
  NetDeviceContainer m_ueDevs;
  FfMacSchedSapProvider* m_hiperCellSchedSapProvider;
  bool m_updateStatusForCentralized;

  void DoSetCellId(uint16_t cellId);
  void DoSetListEnbRrc(Ptr<LteEnbRrc> rrc);
  void DoSetListEnbMac(Ptr<LteEnbMac> mac);
  void SetUeDevices(NetDeviceContainer ueDevs);
  void UpdateRlcBufferStatusForCentralized(FfMacSchedSapProvider* hiperCellSapProvider);
  std::map <uint16_t, std::map<uint8_t, LteMacSapUser*> > GetRlcAttached ();
  ////////////////////////////////
  ...
  LteEnbCmacSapUser* GetLteEnbCmacSapUser (void);
...
private:
  ...
  */
  ...
  void DoAddLc (LteEnbCmacSapProvider::LcInfo lcinfo, LteMacSapUser* msu);
	  ...
	  void DoAddLcCentralizado (LteEnbCmacSapProvider::LcInfo lcinfo, LteMacSapUser* msu);
	  void DoSetUe (uint16_t rnti);
	  /////////////////////////////////////
