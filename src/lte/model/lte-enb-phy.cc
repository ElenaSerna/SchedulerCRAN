...
LteEnbPhy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::LteEnbPhy")
	...
	.AddAttribute ("Centralizado",
				   "Indica si se trata de un escenario centralizaod",
				   BooleanValue (false),
				   MakeBooleanAccessor (&LteEnbPhy::m_centralizado),
				   MakeBooleanChecker ())
  ;
  return tid;
}
...
LteEnbPhy::StartSubFrame (void)
{
              ...
              for (uint8_t i = 0; i < dci->GetDci ().m_mcs.size (); i++)
                {
                  ...
                  ///// INCLUIDO PARA SCHEDULER CENTRALIDO //////
                  if(m_centralizado)
                      params.m_imsi = dci->GetDci().m_imsi;
                  else
                	  params.m_imsi=0;
                  //////////////////////////////////////////////
  ...
  // trigger the MAC
  m_enbPhySapUser->SetCellId(this->m_cellId); // AÑADIDO para conocer la celda sobre la que se ejecuta el scheduler
...
LteEnbPhy::SendDataChannels (Ptr<PacketBurst> pb)
{
  ...
  ctrlMsgList.clear ();
//  m_downlinkSpectrumPhy->SetScenario(m_centralizado);
