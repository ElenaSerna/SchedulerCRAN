...
void
UeManager::InitializeNewUe ()
{

  // Identificamos si se trata de un scheduler centralizado
	m_centralizado=true;
  ...
  m_rrc->m_cmacSapProvider->SetUe (m_rnti); ///// m_rnti¿¿
  m_rrc->m_cphySapProvider->AddUe (m_rnti);
  ...
  // setup the eNB side of SRB0
  {
    ...
    lcinfo.lcId = lcid;
    // leave the rest of lcinfo empty as CCCH (LCID 0) is pre-configured
    m_rrc->m_cmacSapProvider->AddLcCentralizado(lcinfo, rlc->GetLteMacSapUser ());
...
UeManager::SetupDataRadioBearer (EpsBearer bearer, uint8_t bearerId, uint32_t gtpTeid, Ipv4Address transportLayerAddress)
{
  ...
  /// Este paso lo haremos solo si se trata de un scheduler distribuido
  if (!m_centralizado)
...
LteEnbRrc::GetTypeId (void)
{
  ...
  static TypeId tid = TypeId ("ns3::LteEnbRrc")
	...
	.AddTraceSource ("ResetUeTraces",
					 "delete any reference to ues referenced by UeMap",
					 MakeTraceSourceAccessor (&LteEnbRrc::m_resetUeTraces))
					 /////// AÑADIDO PARA CENTRALIZADO ///////
	.AddAttribute ("ConnectTracesFirstTimeENB",
				   "INdica si las trazas se conectan por primera vez para esta eNB",
				   BooleanValue (true), // unused, read-only attribute
				   MakeBooleanAccessor (&LteEnbRrc::m_connectingTracesFirstTime),
				   MakeBooleanChecker())
	/////////////////////////////////////
...
//// Añadido para centralizado ////
// En el algoritmo de scheduler centralizado, añadiremos forzosamente el UE a la celda (RU) seleccionada, utilizando este metodo //
void
LteEnbRrc::SetUe (uint16_t rnti, uint64_t imsi)
{

  Ptr<UeManager> ueManager = CreateObject<UeManager> (this, rnti, UeManager::CONNECTED_NORMALLY);
...
void
LteEnbRrc::ConnectTraces(uint64_t imsi, uint16_t rnti)
{
//	 if (m_connectingTracesFirstTime)
//	 {
		  /// Creamos las trazas rlc con los datos de la celda - rnti/imsi
		  m_resetUeTraces(imsi);
		  m_newUeContextTrace (m_cellId, rnti); // Creamos el contexto celda-imsi
		  m_connectionReconfigurationTrace (imsi, m_cellId, rnti); // creamos traza connection config de la enb

//		  m_connectingTracesFirstTime=false;
//	 }
//	m_handoverStartTrace (imsi, 0, rnti, m_cellId);
//	m_handoverEndOkTrace (imsi, m_cellId, rnti);
}

void
LteEnbRrc::SetUeMap(std::map<uint16_t, Ptr<UeManager>> map)
{
	m_ueMap=map;
}
...
LteEnbRrc::GetIdentifierTable()
{

	std::map <uint16_t, uint64_t> IdTable;
	std::map <uint16_t, Ptr<UeManager>>::iterator itUeMap;
	std::map <uint16_t, uint64_t>::iterator itTable;
	uint16_t rnti;
	uint64_t imsi;
	for (itUeMap=m_ueMap.begin(); itUeMap!=m_ueMap.end(); itUeMap++)
	{
		if ((*itUeMap).second->GetState()==UeManager::INITIAL_RANDOM_ACCESS)
			continue;
		else
		{
			rnti=(*itUeMap).first;
			imsi=(*itUeMap).second->GetImsi();
			itTable=IdTable.find(rnti);
			if (itTable==IdTable.end())
				IdTable.insert(std::pair <uint16_t, uint64_t> (rnti, imsi));
			else
				(*itTable).second=imsi;
		}
	}
...
void
LteEnbRrc::RemovePrevUe (uint16_t rnti)
{
  NS_LOG_FUNCTION (this << (uint32_t) rnti);
  std::map <uint16_t, Ptr<UeManager> >::iterator it = m_ueMap.find (rnti);
