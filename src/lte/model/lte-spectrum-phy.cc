...
void
LteSpectrumPhy::SetScenario(bool centralizado)
{
	m_centralizado=centralizado;
}
  ...
  if (lteDataRxParams != 0)
    {
      ...
      /// CCOMPROBAR QUE TIPO DE MENSAJE SE RECIBE //////
      std::cout << "*** LteSpectrumPhy tamaño de la lista de mensajes de control: " << lteDataRxParams->ctrlMsgList.size() << std::endl;
      std::list<Ptr<LteControlMessage> >::iterator it;
      for (it = lteDataRxParams->ctrlMsgList.begin (); it != lteDataRxParams->ctrlMsgList.end (); it++)
        {
          Ptr<LteControlMessage> msg = (*it);
          std::cout << " * Tipo de Mensaje: " << msg->GetMessageType () << std::endl;
        }
      //////////////////////////////////////////////////
    ...
    {
	  std::cout << " Celda en StartRx: " << lteDlCtrlRxParams->cellId << "----> Celda config. en SpectrumPhy: " << m_cellId  << std::endl;

      m_interferenceCtrl->AddSignal (rxPsd, duration);

      /// CCOMPROBAR QUE TIPO DE MENSAJE SE RECIBE //////
//      std::cout << "*** LteSpectrumPhy tamaño de la lista de mensajes de control: " << lteDlCtrlRxParams->ctrlMsgList.size() << std::endl;
//      std::cout << " Celda config. en SpectrumPhy: " << m_cellId << std::endl;
      std::list<Ptr<LteControlMessage> >::iterator it;
      for (it = lteDlCtrlRxParams->ctrlMsgList.begin (); it != lteDlCtrlRxParams->ctrlMsgList.end (); it++)
        {
          Ptr<LteControlMessage> msg = (*it);
//          std::cout << " * Tipo de Mensaje: " << msg->GetMessageType () << std::endl << " estado: " << m_state << std::endl;
        }
      ...
      case RX_DATA:
        ...
        // simultaneously (e.g., at the eNB).
        {
          ...
          //  LteSpectrumSignalParametersDataFrame
        	...
        	// INCLUIMOS QUE SE PUEDA RECIBIR DATOS DESDE OTRA CELDA QUE NO SEA LA CELDA A LA QUE EL UE
        	// ESTA ATTACHADO --> AÑADIDO PARA SCHEDULER CENTRALIZADO
          if (params->cellId  == m_cellId) //||( m_centralizado && (params->cellId!=m_cellId)))
            {
  ...
  switch (m_state)
    ...
    case IDLE:
      ...
      switch (m_state)
        ...
        case IDLE:
          if (cellId  == m_cellId)// || (m_centralizado && cellId!=m_cellId)) // AÑADIMOS QUE PUEDA RECIBIR INFORMACION DE CONTROL ENVIADA POR OTRA CELDA
            {
      ...
      if (!m_ltePhyRxCtrlEndOkCallback.IsNull ())
        {
          ...
          std::cout << "*** EndRxDlCtrl: Tamaño de la lista de mensajes de control: " << m_rxControlMessageList.size() << std::endl;
		  std::list<Ptr<LteControlMessage> >::iterator itControl;
		  for (itControl = m_rxControlMessageList.begin (); itControl != m_rxControlMessageList.end (); itControl++)
			{
			  Ptr<LteControlMessage> msg = (*itControl);
			  std::cout << " -----> Tipo de Mensaje: " << msg->GetMessageType () << std::endl;
			}
          m_ltePhyRxCtrlEndOkCallback (m_rxControlMessageList);
        }
