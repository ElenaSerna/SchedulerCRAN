...
//      if (Simulator::Now () > m_p10CqiLast + m_p10CqiPeriocity)
	  if (Simulator::Now () >= m_p10CqiLast + m_p10CqiPeriocity) // CQIs cada ms - subtrama
        {
...
//      if  (Simulator::Now () > m_a30CqiLast + m_a30CqiPeriocity)
	  if (Simulator::Now () >=  m_a30CqiLast + m_a30CqiPeriocity) // CQIs cada ms - subtrama
...
//      std::cout << " instante P10: " << Simulator::Now() << " rnti " << m_rnti << std::endl;
...
//      std::cout << " instante A30: " << Simulator::Now() << " rnti " << m_rnti << std::endl;
//      int now = Simulator::Now().GetMilliSeconds();
//
//      FILE * pFile;
//      pFile = fopen ("CQIs.txt","a");
//      fprintf(pFile, "Instante de tiempo \t Subbandas\t");
//	  int rbgNum = cqi.size()/rbgSize;
...
//      fprintf(pFile,"%d\t", now);
//
//	  fprintf(pFile,"RNTI UE %d\t",m_rnti);
//
//
//	  for(int i=0; i<rbgNum; i++)
//	  {
//
//		fprintf(pFile, "  %d\t",rbgMeas.m_higherLayerSelected.at(i).m_sbCqi.at(0)); // Solo un cqi porque solo hay una layer
//
//	  }
//	  fprintf(pFile,"\n");
//	  fclose(pFile);
...
LteUePhy::ReceiveLteControlMessageList (std::list<Ptr<LteControlMessage> > msgList)
{
  ...
  std::cout << "*** LteUePhy Tamaño de la lista de mensajes de control " << msgList.size() <<std::endl;
...
LteUePhy::ReceivePss (uint16_t cellId, Ptr<SpectrumValue> p)
{
  ...
  /*
   * Collect the PSS fofr later processing in GenerateCtrlCqiReport()
...
void
LteUePhy::SynchronizeWithTargetEnb (uint16_t cellId)
{
	  if (cellId == 0)
	    {
	      NS_FATAL_ERROR ("Cell ID shall not be zero");
