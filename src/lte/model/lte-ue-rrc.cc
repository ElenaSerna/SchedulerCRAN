...
LteUeRrc::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::LteUeRrc")
    ...
    .AddTraceSource ("HandoverEndError",
                     ...
                     "ns3::LteUeRrc::ImsiCidRntiTracedCallback")
    ///////// AÑADIDA PARA CENTRALIZADO ///////////
	.AddTraceSource ("Disconnect",
					 "Desconecta las trazadas previamente creadas para este ue",
					 MakeTraceSourceAccessor (&LteUeRrc::m_disconnect))

	.AddAttribute ("ConnectTracesFirstTimeUE",
				   "INdica si las trazas se conectan por primera vez para estE UE",
				   BooleanValue (true), // unused, read-only attribute
				   MakeBooleanAccessor (&LteUeRrc::m_connectingTracesFirstTime),
				   MakeBooleanChecker())
...
void
LteUeRrc::UpdateConnectedTraces(uint16_t cellId)
{
	/// ¿RESETEAR TRAZAS ANTERIORES? ///
//	if(m_connectingTracesFirstTime)
//	{
		m_randomAccessSuccessfulTrace (m_imsi, cellId, m_rnti); // Conectamos la traza que indica que el acceso inicial (a la celda objetivo) se ha realizado correctamente
		// Desconectamos trazas anteriores previamente configuradas en los ues para la hipercelda (o desde otra celda desde la que se transmitio)
	//	(std::string context, uint64_t imsi, uint16_t cellId, uint16_t rnti)
	//    m_handoverStartTrace (m_imsi, 0, m_rnti, cellId);
		m_disconnect(m_imsi, 1, m_rnti);
		m_connectionReconfigurationTrace (m_imsi, cellId, m_rnti); // Conectamos la traza que indica que existe una conexion activa entre la celda objetivo y el UE (= CONNECTED NORMALLY)
